...
function SalaryTab({ showToast }) {
  ...
  const grouped = staffList
    ...
    .filter(g => g.salaries.length > 0 || staffFilter === "sin");

  const filtered = staffFilter
    ? grouped.filter(g =>
        staffFilter === "sin"
          ? g.salaries.length === 0
          : g.staff.idPersonal === parseInt(staffFilter)
      )
    : grouped;
  ...
  const validateForm = () => {
    ...
    if (!formData.tarifaHora)        e.tarifaHora        = "Obligatorio";
    if (!formData.horasTrabajadas)   e.horasTrabajadas   = "Obligatorio";
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setShowForm(false);
      setFormData({ idPersonal: "", tarifaHora: "", horasTrabajadas: "", fechaLiquidacion: "" });
      fetchAll();
    } catch (err) {
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`/salary/${confirmDel.idSalario}`, { headers: auth() });
      showToast("Liquidación eliminada.");
      setConfirmDel(null);
      fetchAll();
    } catch (err) {
      showToast(err?.response?.data?.msg || "No se pudo eliminar.", "error");
      setConfirmDel(null);
    }
  };

  const totalBruto = (sal) =>
    sal.tarifaHora && sal.horasTrabajadas
      ? `$${(parseFloat(sal.tarifaHora) * parseInt(sal.horasTrabajadas)).toLocaleString("es-AR")}`
      : "—";

  const roleDeSalario = (sal) => {
    const s = staffById(sal.idPersonal);
    if (!s) return null;
    ...
    <div>
      {confirmDel && (
        <ConfirmModal
          ...
          message={`Se eliminará la liquidación del ${confirmDel.fechaLiquidacion}. Esta acción no se puede deshacer.`}
          onConfirm={handleDelete}
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <button
          ...
          style={{ padding: "9px 18px", borderRadius: 8, border: "none", background: VET_COLORS.accent, color: "white", cursor: "pointer", fontWeight: 700, fontSize: 13, boxShadow: "0 2px 8px rgba(45,106,79,0.25)" }}
        >
          + Nueva liquidación
        </button>
      </div>
      ...
      {showForm && (
        <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 12, padding: "20px 24px", marginBottom: 20 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#166534", marginBottom: 16 }}>
            📝 Registrar nueva liquidación
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <Field label="Personal" required error={formErrors.idPersonal} colSpan="1 / -1">
              <select
                style={inputStyle(formErrors.idPersonal)}
                value={formData.idPersonal}
                onChange={e => setFormData(p => ({ ...p, idPersonal: e.target.value }))}
              >
            ...
            <Field label="Período (fecha de liquidación)" required error={formErrors.fechaLiquidacion} colSpan="1 / -1">
              <input
                type="date"
                style={inputStyle(formErrors.fechaLiquidacion)}
                value={formData.fechaLiquidacion}
                onChange={e => setFormData(p => ({ ...p, fechaLiquidacion: e.target.value }))}
              />
            </Field>
            <Field label="Tarifa por hora ($)" required error={formErrors.tarifaHora}>
              <input
                type="number"
                style={inputStyle(formErrors.tarifaHora)}
                value={formData.tarifaHora}
                onChange={e => setFormData(p => ({ ...p, tarifaHora: e.target.value }))}
                placeholder="ej: 1800"
              />
            </Field>
            <Field label="Horas trabajadas" required error={formErrors.horasTrabajadas}>
              <input
                type="number"
                style={inputStyle(formErrors.horasTrabajadas)}
                value={formData.horasTrabajadas}
                onChange={e => setFormData(p => ({ ...p, horasTrabajadas: e.target.value }))}
                placeholder="ej: 160"
              />
            </Field>
          </div>
          ...
          {/* Preview total */}
          {formData.tarifaHora && formData.horasTrabajadas && (
            <div style={{ marginTop: 14, padding: "12px 16px", background: "white", borderRadius: 8, border: "1px solid #bbf7d0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{ fontSize: 13, color: "#166534" }}>Total bruto estimado:</span>
              <span style={{ fontSize: 18, fontWeight: 800, color: "#166534" }}>
                ${(parseFloat(formData.tarifaHora) * parseInt(formData.horasTrabajadas)).toLocaleString("es-AR")}
              </span>
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 16, justifyContent: "flex-end" }}>
            <button
              ...
              style={{ padding: "9px 20px", borderRadius: 8, border: `1px solid ${VET_COLORS.border}`, background: "white", cursor: "pointer", fontWeight: 600, fontSize: 13 }}
            >
              Cancelar
            </button>
            <button
              ...
              disabled={saving}
              style={{ padding: "9px 24px", borderRadius: 8, border: "none", background: saving ? "#94a3b8" : VET_COLORS.accent, color: "white", cursor: saving ? "not-allowed" : "pointer", fontWeight: 700, fontSize: 13 }}
            >
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {filtered.map(({ staff: s, salaries: sals }) => {
            const rol = roleDeSalario(sals[0] || { idPersonal: s.idPersonal });
            const rolM = ROLE_META[
              s.Admin ? 1 : s.Veterinarian ? 2 : s.Assistant ? 3 : s.Seller ? 4 : 0
            ] || { label: "—", color: "#64748b", bg: "#f1f5f9" };
            return (
              <div key={s.idPersonal} style={{ background: "white", border: `1px solid ${VET_COLORS.border}`, borderRadius: 12, overflow: "hidden" }}>
                ...
                <div style={{ padding: "14px 18px", background: "#f8fafc", borderBottom: `1px solid ${VET_COLORS.border}`, display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 99, background: rolM.bg, color: rolM.color }}>
                    {rolM.label}
                  </span>
                  <span style={{ fontSize: 11, fontWeight: 600, color: VET_COLORS.textMuted }}>
                    {sals.length} liquidación{sals.length !== 1 ? "es" : ""}
                  </span>
                </div>

                {/* Tabla de liquidaciones */}
                {sals.length === 0 ? (
                  <div style={{ padding: "16px 18px", fontSize: 13, color: "#94a3b8" }}>Sin liquidaciones registradas.</div>
                ) : (
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    <thead>
                      <tr style={{ background: "#fafafa" }}>
                        {["Período", "Tarifa/hora", "Horas", "Total bruto", ""].map((h, i) => (
                    ...
                    <tbody>
                      {sals.map((sal, idx) => (
                        <tr key={sal.idSalario}
                          style={{ borderBottom: idx < sals.length - 1 ? `1px solid ${VET_COLORS.border}` : "none", background: idx === 0 ? "#f0fdf4" : "white" }}
                        >
                          <td style={{ padding: "11px 16px", fontWeight: idx === 0 ? 700 : 400 }}>
                            {sal.fechaLiquidacion || "—"}
                          ...
                          <td style={{ padding: "11px 16px", color: "#475569" }}>${sal.tarifaHora ?? "—"}/h</td>
                          <td style={{ padding: "11px 16px", color: "#475569" }}>{sal.horasTrabajadas ?? "—"} hs</td>
                          <td style={{ padding: "11px 16px", fontWeight: 600, color: "#166534" }}>{totalBruto(sal)}</td>
                          <td style={{ padding: "11px 16px", textAlign: "right" }}>
                            <button
                              onClick={() => setConfirmDel(sal)}
                              style={{ padding: "4px 10px", borderRadius: 6, fontSize: 11, fontWeight: 600, border: "1px solid #fecaca", background: "#fff5f5", color: "#c62828", cursor: "pointer" }}
                            >
                              Eliminar
                            </button>
                          </td>
      ...
      <p style={{ margin: "12px 0 0", fontSize: 11, color: "#94a3b8" }}>
        💡 Cada fila es una liquidación mensual independiente. La más reciente se marca como ÚLTIMO.
      </p>
    </div>
  );
}

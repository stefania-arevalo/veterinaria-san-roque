...
export default function StaffCreateDrawer({ onClose, onSaved, localities }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const staffRes   = await axios.post(apiUrl("/staff"), staffPayload, { headers: authHeaders() });
      const idPersonal = staffRes.data.idPersonal;

      // 2. Crear datos del rol
      if (selectedRole === 2) await axios.post(apiUrl("/veterinarian"), { idPersonal, especialidad: roleForm.especialidad, idMatricula: parseInt(roleForm.matricula) }, { headers: authHeaders() });
      else if (selectedRole === 3) await axios.post(apiUrl("/assistant"), { idPersonal, certificados: roleForm.certificados || null }, { headers: authHeaders() });
      else if (selectedRole === 4) await axios.post(apiUrl("/seller"), { idPersonal }, { headers: authHeaders() });
      else if (selectedRole === 1) await axios.post(apiUrl("/admin"), { idPersonal, areaResponsabilidad: roleForm.areaResponsabilidad }, { headers: authHeaders() });
...
export function StaffEditDrawer({ staff, localities, onClose, onSaved }) {
  ...
  const handleSavePersonal = async () => {
    ...
    try {
      await axios.patch(apiUrl(`/staff/${staff.idPersonal}`), form, { headers: authHeaders() });
  ...
  const handleSaveRol = async () => {
    ...
    setSaving(true); setGlobalErr("");
    try {
      if (idRol === 2 && vet) await axios.patch(apiUrl(`/veterinarian/${staff.idPersonal}`), { especialidad: vetForm.especialidad, idMatricula: parseInt(vetForm.idMatricula) }, { headers: authHeaders() });
      else if (idRol === 3 && asst) await axios.patch(apiUrl(`/assistant/${staff.idPersonal}`), { certificados: asstForm.certificados || null }, { headers: authHeaders() });
      else if (idRol === 1 && adm) await axios.patch(apiUrl(`/admin/${staff.idPersonal}`), { areaResponsabilidad: admForm.areaResponsabilidad }, { headers: authHeaders() });
  ...
  const handleSaveAcceso = async () => {
    ...
    try {
      if (!hasUser) {
        ...
        await axios.patch(apiUrl(`/user/${staff.User.idUsuario}`), payload, { headers: authHeaders() });
      }
